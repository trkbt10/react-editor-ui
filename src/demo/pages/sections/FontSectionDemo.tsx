/**
 * @file FontSectionDemo - Demo page for FontSection
 */

import { useState, type CSSProperties } from "react";
import { FontSection } from "../../../sections/FontSection/FontSection";
import type { FontData } from "../../../sections/FontSection/types";

const containerStyle: CSSProperties = {
  padding: 24,
  maxWidth: 400,
};

const sectionWrapperStyle: CSSProperties = {
  padding: 16,
  backgroundColor: "var(--rei-color-surface)",
  borderRadius: 8,
  border: "1px solid var(--rei-color-border)",
};

const stateDisplayStyle: CSSProperties = {
  marginTop: 12,
  padding: 8,
  backgroundColor: "var(--rei-color-surface-raised)",
  borderRadius: 4,
  fontSize: 11,
  fontFamily: "monospace",
  color: "var(--rei-color-text-muted)",
  whiteSpace: "pre-wrap",
};

/**
 * FontSection demo page.
 */
export function FontSectionDemo() {
  const [data, setData] = useState<FontData>({
    family: "Inter",
    weight: "400",
  });

  return (
    <div style={containerStyle}>
      <div style={sectionWrapperStyle}>
        <FontSection data={data} onChange={setData} />
        <div style={stateDisplayStyle}>{JSON.stringify(data, null, 2)}</div>
      </div>
    </div>
  );
}
